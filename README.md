...
* `fallback` - when truthy, another translation is returned for fields that are not available in the currently selected language (default: `false`, i.e. fields with missing translation are returned as `null`)
  * if configured with the special value `'variants'`, the closest matching ISO 639 variant (e.g. region/dialect) to the selected language is returned (e.g. `en` for `en-GB`, `en-US` etc.)
  * with any truthy config option (or if there is no matching variant), the fallback order is determined by the order of the `languages` option (i.e. the first available translation is returned)
